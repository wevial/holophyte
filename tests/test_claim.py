...
class OffBoardMirrorTests(LoopFixture):
    """KO-425: a mirror row follows its ticket off the board's ready
    column, so the supervisor stops counting it.

    The store mirrors a ticket when a pass sees it ready, and nothing used
    to move the row when the ticket left the column unclaimed -- pulled to
    Backlog, say, or blocked by a new relation. The row stayed `ready`,
    `ready_tickets()` counted it as owed a loop on every sweep, and each
    relaunched pass found the board empty and exited. The empty pass now
    walks each such row to `blocked_on_operator` and names them in one
    line; a `--requeue` or a drag back to Todo walks it to `ready` again,
    as before.
    """
    ...
    def test_a_ready_row_whose_ticket_left_the_board_is_parked_and_named(self):
        """KO-134 was mirrored `ready` and ran once; the operator then
        pulled it to Backlog. The pass finds the board's ready column
        empty, walks the stale row to `blocked_on_operator` and names it,
        with the `note` on its last run -- the sweep owes it nothing."""
