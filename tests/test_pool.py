...
class PoolTests(PoolRestartCases, LoopFixture):
    ...
    def test_stop_on_failure_false_keeps_spawning_and_exits_zero(self):
        provider = StubProvider(*(a_task(n) for n in range(1, 4)))
    ...
    def test_an_old_handoff_failure_does_not_poison_idle_drain(self):
        ...
        pool = self.run_scheduler(2, StubProvider(), [])

        self.assertEqual(pool.spawned, [])
        self.assertEqual(self.rc, 0)
        self.assertIn("Linear has no ready tickets. done.", self.out)

    def test_a_listing_failure_with_a_live_worker_can_recover(self):
        provider = StubProvider(a_task(1))
        ready = provider.ready_issues
