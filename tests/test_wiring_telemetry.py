...
class ReportTests(ReportStoreCase):
    ...
    def test_rejected_run_has_its_own_report_column(self):
        self.completed_run(1, actual_min=5, estimate_min=25, rounds=1,
                           outcome="rejected")
        # Past the live block: its "in flight" line, the two toil lines and
        # the blank.
        lines = holophyte.report.report_lines(self.conn)[4:]
    ...
    def test_a_line_per_run_with_its_ratio_and_a_summary(self):
        ...
        lines = holophyte.report.report_lines(self.conn)[5:]
    ...
    def test_report_prints_the_table_and_claims_nothing(self):
        ...
        # The failure count and the toil lines precede the table, and below
        # it the `[report] findings` mode -- the default, nothing configured
        # -- and the one line on the supervisor: none has ever beaten here.
        self.assertEqual(printed[:10], holophyte.report.report_lines(self.conn))
        self.assertEqual(printed[10], "findings: none")
        self.assertEqual(printed[11], "supervisor: none recorded")
        self.assertEqual(len(printed), 12)
...
class HostLabelTests(ReportStoreCase):
    ...
    def test_the_report_and_findings_show_the_label_and_never_the_hostname(self):
        ...
        self.assertEqual([line.split()[-1] for line in printed[7:10]],
