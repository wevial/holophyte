...
test("the bar keeps its proportional widths, remainder and running pulse; the status line names the running phase", () => {
  render(<RoundTimeline segments={SEGMENTS} />);
...
test("a finished timeline's status line reads done with the run's whole span", () => {
  const ended: Segment[] = [
    { kind: "implement", label: "implement", from: T, to: T + 60 * MINUTE, running: false, width: 0.5 },
    { kind: "merge", label: "merge", from: T + 60 * MINUTE, to: T + 82 * MINUTE, running: false, width: 0.5 },
  ];
  render(<RoundTimeline segments={ended} />);
