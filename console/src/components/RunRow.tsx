    ...
    <li data-run={run.id} className="border-t border-line-faint">
      ...
      >
        ...
        <span className="flex min-w-0 items-center gap-2">
          ...
          {run.stop_requested && <span title={run.stop_requested} className="min-w-0 truncate text-[12px] text-muted">{run.stop_action?.startsWith("abort") ? "Abort" : "Pause"} requested: {run.stop_requested}</span>}
