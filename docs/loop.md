...
2. Every project refreshes `main` from origin before every cut: `git fetch
   ...
   enables it with `extensions.worktreeConfig = true` and worktree-local
   `core.hooksPath = .githooks`, before any agent turn. Hook configuration
   failure fails setup.
...
4. Verify gate: the ticket's mechanical verify command must pass before
   ...
   parseable top-level `&&` chain or instrumented simple newline block runs
   clause by clause in one shell,
   ...
   short-circuited
   — silence is reported as silence, never as a bare non-zero exit. Complex
...
one of the three exits. Under `[board] mode = "store"` the scheduler counts
