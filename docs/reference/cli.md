...
| `--serve PORT TARGET` | the read-only JSON daemon on loopback (`--serve 7710` binds `127.0.0.1:7710`), which also serves the console at `/` from the built bundle; `--serve HOST:PORT` binds the named address instead and then demands `[serve] token_file`, whose contents every request must present as a bearer token | store, read-only |
