...
| `--report PROJECT` | prints the estimate-vs-actual table and the supervisor liveness line | store, read-only |
| `--sweep PROJECT` | prints what an acting sweep would do; lists stray review containers | store (sightings only) |
| `--sweep --act PROJECT` | fails tripped runs, releases their leases, removes stray containers | store, Docker |
...
`--babysit` and `--file-ticket` need a `[board]` table. The loop additionally live-probes each configured agent
...
| 1 | a startup refusal, a failed run under `stop_on_failure`, an invalid ticket file, a refused requeue, approval, babysitter or re-point |
