...
def reconcile_parked_pull_requests(target, conn, now, provider=None, out=None,
    ...
    (`store.read.pending_loop_launches()`): the babysitter row
    `store.shepherd()` wrote for the send-back, on a ticket still
    `ready`, with no `launch_loop` row since. A
...
def start_loop_for(target, conn, owed, now, out):
    ...
    attempt = (f"the supervisor is starting {unit} for {count} {noun} sent"
               " back to the babysitter while no loop is live")
    with store.transaction(conn):
    ...
    note = (f"the supervisor started {unit} for {count} {noun} sent back to"
            " the babysitter while no loop was live")
    with store.transaction(conn):
        for _ticket, run_id in owed:
