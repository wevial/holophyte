...
def acknowledge(target, conn, run_id, pull, threads, handle):
    """React EYES once to the latest comment of each thread that mentions
    `handle` (KO-679). One the route's account already reacted to, or a
    bot's, is left alone; a refused reaction is a run event, never fatal."""
    pattern = _mention(handle)
