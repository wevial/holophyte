...
The same review's size gate held `holophyte/serve.py` to its 1,000-line
ceiling and the named region was ~140 lines of the 1,487, so the
`/runs`, `/runs/N`, `/runs/N/files`, `/runs/N/ledger`, `/shipped` and
`/ledger` read routes moved here with it, verbatim and in their old
order, with the helpers only they call -- `no_store()` and `json_host()`
came along rather than leave the moved bodies reaching back into
`holophyte.serve`, so the import runs one way: `holophyte.serve` keeps
the board views, the token check and the server, and names this module's
handlers in its dispatch.
"""
