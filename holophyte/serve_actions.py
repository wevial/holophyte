...
`no_store()`, shared with the read routes, stays home: `holophyte.serve`
imports this module's names for its dispatch, so `requeue_action()`
reaches it back through a deferred `from holophyte.serve import`, the
way `holophyte.serve_config`'s `_write_config()` reaches
`record_action_intervention()` here.
"""
...
def requeue_action(target, body):
    ...
    from holophyte.serve import no_store
    if not target.store_path.exists():
