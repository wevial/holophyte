...
`--report`, `--status`, `--sweep` and `--serve` read the store and call
nobody; the loop and `--supervise` need a `[board]` table. `--help` is safe:
the command line is parsed, not indexed.
